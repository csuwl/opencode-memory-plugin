 ...
 */
async function getOpenAIEmbedding(text, apiKey, model = 'text-embedding-3-small', baseURL) {
    const url = baseURL
    ...
    try {
        switch (embedding.provider) {
            case 'openai': {
                ...
                if (!apiKey) {
                    console.warn('OpenAI API key not found (set OPENAI_API_KEY env var or configure in memory-config.json). ' +
            ...
            case 'local':
                ...
                console.warn('Local embedding not yet implemented. ' +
                    'Falling back to hash-based embedding.');
                embeddingVector = hashEmbedding(text);
                break;
            case 'none':
